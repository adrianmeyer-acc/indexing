<?php
  ...
  function fetchLetter( $aLetter, &$names ) {
    ...
    do {
			...
			$url = "http://surnames.behindthename.com/names/letter/{$aLetter}";
      if ($pageNumber > 1) {
				$url.= "/{$pageNumber}";
			}
  ...
  function fetchSynonyms( $aName ) {
		$parser = new HtmlParser();
		$url = "http://surnames.behindthename.com/php/extra.php?extra=r&terms=".
...
//  fetchLetter( 'q', $names );
  ...
  $tabFile = fopen("behindthename-surname-all.txt", "w");
  fwrite( $tabFile, "\xEF\xBB\xBF" );
	fwrite( $tabFile, "name\tsynonyms\tlanguages\n" );

  $synonymFile = fopen("behindthename-synonyms-surname.csv", "w");
  fwrite( $synonymFile, "\xEF\xBB\xBF" );
	fwrite( $synonymFile, "name,".implode(',',$synonymHeader)."\n" );
